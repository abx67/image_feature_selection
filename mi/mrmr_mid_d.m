...
t1=cputime;
for i=1:nd
   t(i) = mutualinfo(d(:,i), f);       %t(i)表示第i列对其他列的互信息(列向量）
end; 
fprintf('calculate the marginal dmi costs %5.1fs.\n', cputime-t1);

[tmp, idxs] = sort(-t);     %tmp互信息矩阵t排序,idxs返回排序后元素原来的位置
fea_base = idxs(1:K);       %fea表示互信息从大到小的变量原来的位置

fea(1) = idxs(1);           

KMAX = min(1000,nd); %500      %最多1000个变量

idxleft = idxs(2:KMAX);     %互信息从大到小排序的原来位置，除去第一个
...
for k=2:K,
   t1=cputime;
   ncand = length(idxleft);     %当前循环的变量个数
   curlastfea = length(fea);    %变量总个数
   for i=1:ncand,
